...
public class PortValidation extends Validation {
	...
	public PortValidation(TextField textField, StringProperty port) {
		...
		textField.setTextFormatter(new TextFormatter<String>(c -> {
			...
			if (c.getControlNewText().matches("\\d*")) {
				...
				// Check if another Port is same
				checkPort(textField);
				
				
				// not more chars, if maxNumber need
				if (c.getControlNewText().length() > (""+maxPort).length()) {
					return null;
				}
	...
	private void checkPort(TextField textField) {
		...
		if (!textField.getText().contentEquals("0")) {
			...
			if (count == 1) {
				...
				try {
					socket = new ServerSocket(Integer.parseInt(textField.getText()));
				} 
				catch (IOException e) {

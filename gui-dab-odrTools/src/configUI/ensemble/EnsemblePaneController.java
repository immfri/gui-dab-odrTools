...
public class EnsemblePaneController implements Initializable {
	...
	@FXML Spinner<Integer> framesSpinner; 
	@FXML TextField telnetTextField, managementTextField;
	@FXML CheckBox syslogCheckBox, timestampCheckBox, sccaCheckBox;
	
	// Ensemble
	@FXML Label label, shortLabel, idLabel, countryLabel, offsetLabel;
	@FXML TextField labelTextField, shortLabelTextField, idTextField;
	@FXML Spinner<String> offsetSpinner;
	@FXML ChoiceBox<String> countryChoiceBox;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		// ETI-Frames
		new NumberValidation(framesSpinner, mux.getNbframes(), 0, Integer.MAX_VALUE, 1, nonstopLabel);
		
		// Telnet-Port
		new PortValidation(telnetTextField, mux.getTelnetPort());
		
		// Management-Port
		new PortValidation(managementTextField, mux.getManagementport());
		...
		// Country
		countryChoiceBox.setItems(ensemble.getCountryList());
		countryChoiceBox.valueProperty().bindBidirectional(ensemble.getCountry());
